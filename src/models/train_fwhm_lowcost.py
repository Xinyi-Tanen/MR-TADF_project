"""
MR-TADF FWHM low-cost RF classification model
============================================

Purpose
-------
Build a low-cost FWHM classification model using 30 nm as the boundary:

    y = 1  -> narrow emission, FWHM <= 30 nm
    y = 0  -> broad emission,  FWHM > 30 nm

This script only uses features that can be calculated from SMILES and the
user-provided A/B/C core-fragment structure file:

    - RDKit 2D descriptors
    - ring / rigidity / conjugation descriptors
    - atom-count and functional-group descriptors
    - A/B/C core-fragment motif match/count/similarity descriptors
    - Solution/environment one-hot descriptors, if present
    - Morgan radius-2 fingerprint bits after frequency filtering

The script does not build or use S1, T1, dEST, f, taup, HOMO, LUMO,
total_energy, dipole_D, or any other TDDFT/DFT/experimental excited-state
features.

Main outputs
------------
fwhm_lowcost_classification_outputs/
    all_model_results_summary_fwhm.csv
    lowcost/
        final_fwhm_lowcost_RF_model.pkl
        selected_features_fwhm_lowcost.csv
        all_feature_columns_fwhm_lowcost.csv
        Table_RF_scaffold_metrics_fwhm_lowcost.csv
        Figure_ROC_curve_RF_scaffold_fwhm_lowcost.png
        Figure_confusion_matrix_RF_scaffold_fwhm_lowcost.png
        Figure_feature_importance_RF_fwhm_lowcost_top20.png
        Figure_feature_group_importance_RF_fwhm_lowcost.png
"""
...
OUTPUT_DIR = str(PROJECT_ROOT / "results" / "fwhm_lowcost")
...
def plot_grouped_bar(ax, data_dict, ylabel, subtitle, ylim):
    ...
    for i, (model, color) in enumerate(zip(model_order, colors)):
        values = [data_dict[label].get(model, np.nan) for label in labels]
        bars = ax.bar(
            x + (i - 1) * width,
            values,
            width=width,
        ...
        for bar, v in zip(bars, values):
            if pd.isna(v):
                ...
                label = f"{v:.2f}"
                ypos = v + 0.008
            ax.text(bar.get_x() + bar.get_width() / 2, ypos, label, ha="center", va="bottom", fontsize=10)
...
def feature_to_label(feature: str) -> str:
    label_map = {
        ...
        "MolMR": "MolMR",
        "NumHDonors": r"$N_{HD}$",
        "NumHAcceptors": r"$N_{HA}$",
        "NumRotatableBonds": r"$N_{rot}$",
        "RingCount": r"$N_{ring}$",
        "HeavyAtomCount": r"$N_{heavy}$",
        "NumAromaticRings": r"$N_{ArRing}$",
        "FractionCSP3": r"$f_{CSP3}$",
        "SP2AtomFrac": "SP2AtomFrac",
        "AromaticAtomFrac": "AromaticAtomFrac",
        "HeteroAtomFrac": "HeteroAtomFrac",
    }
...
def plot_feature_importance(fi_df, output_dir, feature_set_name):
    fi_df = fi_df.copy().sort_values("importance", ascending=False)
    fi_df["importance_norm"] = fi_df["importance"] / (fi_df["importance"].sum() + 1e-12)
    fi_df.to_csv(os.path.join(output_dir, f"Table_{FINAL_MODEL_NAME}_feature_importance_fwhm_{feature_set_name}.csv"),
    ...
    ax.barh(fi_top["Feature_label"], fi_top["importance"], color=MAIN_BLUE, edgecolor="none")
    ax.set_xlabel("Feature importance", fontsize=16, labelpad=8)
    ax.set_ylabel("Feature", fontsize=16, labelpad=8)
    ax.set_title(f"Top 20 feature importances ({feature_set_name})", fontsize=17, pad=10)
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"Figure_feature_importance_{FINAL_MODEL_NAME}_fwhm_{feature_set_name}_top20.png"),
    ...
    group_imp.to_csv(os.path.join(output_dir, f"Table_feature_group_importance_fwhm_{feature_set_name}.csv"),
                     index=False, encoding="utf-8-sig")

    fig, ax = plt.subplots(figsize=(8.8, 6.4))
    ax.barh(group_imp["feature_group"], group_imp["importance"], color=MAIN_BLUE, edgecolor="none")
    ax.set_xlabel("Summed feature importance", fontsize=16, labelpad=8)
    ax.set_ylabel("Feature group", fontsize=16, labelpad=8)
    ax.set_title(f"Feature group importance ({feature_set_name})", fontsize=17, pad=10)
    ax.tick_params(axis="both", labelsize=14)
    ax.grid(axis="x", linestyle="--", alpha=0.35)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"Figure_feature_group_importance_{FINAL_MODEL_NAME}_fwhm_{feature_set_name}.png"),
        dpi=DPI, bbox_inches="tight")
    plt.close()
...
def plot_roc_and_confusion(y_true, y_pred, y_prob, output_dir, feature_set_name):
    if len(np.unique(y_true)) == 2:
    ...
    plt.plot(fpr, tpr, lw=2.2, label=f"{FINAL_MODEL_NAME} (AUC = {roc_value:.3f})", color=MAIN_BLUE)
    plt.plot([0, 1], [0, 1], linestyle="--", lw=1.5, color="gray")
    plt.xlim(0.0, 1.0)
    plt.ylim(0.0, 1.05)
    plt.xlabel("False positive rate", fontsize=13)
    plt.ylabel("True positive rate", fontsize=13)
    plt.title(f"ROC curve of {FINAL_MODEL_NAME} under scaffold split\n{feature_set_name}", fontsize=14, pad=10)
    ...
    ax.spines["right"].set_visible(False)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"Figure_ROC_curve_{FINAL_MODEL_NAME}_scaffold_fwhm_{feature_set_name}.png"),
                dpi=DPI, bbox_inches="tight")
    plt.close()
    ...
    ax.set_title(f"Confusion matrix of {FINAL_MODEL_NAME}\nscaffold OOF, {feature_set_name}", fontsize=14, pad=10)
    ax.set_xlabel("Predicted label", fontsize=13)
    ax.set_ylabel("True label", fontsize=13)
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"Figure_confusion_matrix_{FINAL_MODEL_NAME}_scaffold_fwhm_{feature_set_name}.png"),
        dpi=DPI, bbox_inches="tight")
    plt.close()
...
def save_figure(fig, save_path):
    """Save both PNG and PDF into the requested output folder."""
...
def categorize_feature(feature: str) -> str:
    if feature in ["S1", "T1", "dEST", "S1_T1_gap", "f", "taup", "log_taup", "HOMO", "LUMO", "HL_gap", "orbital_center", "f_div_dEST", "f_times_dEST", "total_energy", "dipole_D"]:
...
def plot_feature_importance(fi_df, output_dir, feature_set_name):
    fi_df = fi_df.copy().sort_values("importance", ascending=False)
    fi_df["importance_norm"] = fi_df["importance"] / (fi_df["importance"].sum() + 1e-12)
    fi_df.to_csv(os.path.join(output_dir, f"Table_{FINAL_MODEL_NAME}_feature_importance_fwhm_{feature_set_name}.csv"), index=False, encoding="utf-8-sig")
    ...
    save_figure(fig, os.path.join(output_dir, f"Figure_feature_importance_{FINAL_MODEL_NAME}_fwhm_{feature_set_name}_top20.png"))

    fi_df["feature_group"] = fi_df["feature"].map(categorize_feature)
    group_imp = fi_df.groupby("feature_group", as_index=False)["importance"].sum().sort_values("importance", ascending=True)
    group_imp.to_csv(os.path.join(output_dir, f"Table_feature_group_importance_fwhm_{feature_set_name}.csv"), index=False, encoding="utf-8-sig")
    ...
    prettify_axis(ax, grid_axis="x", labelsize=INTERPRET_TICK_SIZE)
    save_figure(fig, os.path.join(output_dir, f"Figure_feature_group_importance_{FINAL_MODEL_NAME}_fwhm_{feature_set_name}.png"))
...
def plot_roc_and_confusion(y_true, y_pred, y_prob, output_dir, feature_set_name):
    ...
    save_figure(fig, os.path.join(output_dir, f"Figure_ROC_curve_{FINAL_MODEL_NAME}_scaffold_fwhm_{feature_set_name}.png"))
    ...
    ax.set_ylabel("True label", fontsize=13)
    prettify_axis(ax)
    save_figure(fig, os.path.join(output_dir, f"Figure_confusion_matrix_{FINAL_MODEL_NAME}_scaffold_fwhm_{feature_set_name}.png"))


def plot_shap_summary(model_package, X_for_shap: pd.DataFrame, output_dir: str, feature_set_name: str, max_display=20):
    try:
        import shap
    ...
    try:
        ...
        save_figure(fig, os.path.join(output_dir, f"Figure_SHAP_summary_{FINAL_MODEL_NAME}_fwhm_{feature_set_name}.png"))
        ...
        shap_bar_df.to_csv(os.path.join(output_dir, f"Table_SHAP_importance_{FINAL_MODEL_NAME}_fwhm_{feature_set_name}.csv"), index=False, encoding="utf-8-sig")
        ...
        prettify_axis(ax, grid_axis="x", labelsize=INTERPRET_TICK_SIZE)
        save_figure(fig, os.path.join(output_dir, f"Figure_SHAP_bar_{FINAL_MODEL_NAME}_fwhm_{feature_set_name}.png"))
    except Exception as exc:
...
def fit_deployment_model(X_all, y, metadata, output_dir, feature_set_name, top_n):
    selected_features, selector_importance = select_top_features(X_all, y, top_n=top_n)
    selector_importance.to_csv(
        os.path.join(output_dir, f"feature_selection_importance_full_data_{feature_set_name}.csv"), index=False,
        encoding="utf-8-sig")
    ...
    model_package = {
        ...
        "best_threshold": 0.50,
        "recommended_screening_threshold": 0.60,
        "positive_label": f"FWHM <= {FWHM_THRESHOLD} nm",
        "feature_set": feature_set_name,
        "feature_generator": "MR_TADF_FWHM_lowcost.py",
        "note": "Use the same featurization and reindex to selected_features before prediction.",
    }
    model_path = os.path.join(output_dir, f"final_fwhm_{feature_set_name}_{FINAL_MODEL_NAME}_model.pkl")
    joblib.dump(model_package, model_path)

    pd.DataFrame({"feature": selected_features}).to_csv(
        os.path.join(output_dir, f"selected_features_fwhm_{feature_set_name}.csv"), index=False, encoding="utf-8-sig"
    )
    pd.DataFrame({"feature": X_all.columns.tolist()}).to_csv(
        os.path.join(output_dir, f"all_feature_columns_fwhm_{feature_set_name}.csv"), index=False, encoding="utf-8-sig"
    )
    ...
    meta_to_save.update({
        ...
        "feature_set": feature_set_name,
        "feature_config": "base + MORGAN2 only",
    })
    with open(os.path.join(output_dir, f"fwhm_model_metadata_{feature_set_name}.json"), "w", encoding="utf-8") as f:
...
def main():
    ...
    print("FWHM column:", target_col)
    print("Original data size:", len(df))
    print(f"Class definition: y=1 if FWHM <= {FWHM_THRESHOLD} nm; y=0 otherwise")
    print("Class distribution, 0=Broad FWHM, 1=Narrow FWHM:")
    ...
    # Low-cost version: expensive columns may exist in the source table, but they are intentionally not used.
    X_lowcost = X_lowcost.replace([np.inf, -np.inf], np.nan).fillna(0)

    # Optimized low-cost-only feature setting: base descriptors + A/B/C motif features + MORGAN2 only.
    # Do not include expensive features or high-dimensional MORGAN3/MACCS/RDKFP/PATTERN fingerprints.
    fp_prefixes = ("MORGAN2_", "MORGAN3_", "MACCS_", "RDKFP_", "PATTERN_")
    base_lowcost_cols = [c for c in X_lowcost.columns if not c.startswith(fp_prefixes)]
    morgan2_cols = [c for c in X_lowcost.columns if c.startswith("MORGAN2_")]
    X_lowcost = X_lowcost[base_lowcost_cols + morgan2_cols].copy()

    print("Valid molecules:", len(df_valid))
    print("Low-cost feature matrix shape before selection:", X_lowcost.shape)
    ...
    all_results = []
    final_data_by_feature_set = {}

    for feature_set_name, pack in feature_sets.items():
        X = pack["X"]
        top_n = pack["top_n"]
        subdir = os.path.join(OUTPUT_DIR, feature_set_name)
        ensure_dir(subdir)

        random_results, random_selected_features, random_imp = evaluate_random_split(X, y, feature_set_name, top_n)
        ...
        final_data_by_feature_set[feature_set_name] = final_data
        ...
        metrics_df.to_csv(
            os.path.join(subdir, f"Table_{FINAL_MODEL_NAME}_scaffold_metrics_fwhm_{feature_set_name}.csv"), index=False,
            encoding="utf-8-sig")

        print("\n" + "=" * 78)
        print(f"Final OOF model: {FINAL_MODEL_NAME} + scaffold split + {feature_set_name}")
        print("=" * 78)
        print(metrics_df.to_string(index=False, float_format=lambda x: f"{x:.4f}"))
        print("\nClassification report:")
        print(classification_report(y_true, y_pred, target_names=[NEGATIVE_LABEL_NAME, POSITIVE_LABEL_NAME], digits=4))

        plot_roc_and_confusion(y_true, y_pred, y_prob, subdir, feature_set_name)

        final_data["oof_importance"].to_csv(
            os.path.join(subdir,
                         f"Table_{FINAL_MODEL_NAME}_scaffold_OOF_feature_importance_fwhm_{feature_set_name}.csv"),
            index=False,
            encoding="utf-8-sig",
        )

        model_package, deploy_fi = fit_deployment_model(X, y, pack["metadata"], subdir, feature_set_name, top_n)
        plot_feature_importance(deploy_fi, subdir, feature_set_name)
        plot_shap_summary(model_package, X[model_package["selected_features"]], subdir, feature_set_name, max_display=20)

        print("Selected deployment features:", len(model_package["selected_features"]))
        print(model_package["selected_features"])
        print(f"Top 20 deployment {FINAL_MODEL_NAME} features:")
        print(deploy_fi.head(20).to_string(index=False, float_format=lambda x: f"{x:.6f}"))

    df_res = pd.DataFrame(all_results)
    df_res.to_csv(os.path.join(OUTPUT_DIR, "all_model_results_summary_fwhm.csv"), index=False, encoding="utf-8-sig")
    print("\n===== Model comparison summary =====")
    print(df_res.to_string(index=False, float_format=lambda x: f"{x:.4f}"))

    print("\nSaved files in:", OUTPUT_DIR)
    print(" - all_model_results_summary_fwhm.csv")
    print(" - lowcost/final_fwhm_lowcost_{}_model.pkl".format(FINAL_MODEL_NAME))
